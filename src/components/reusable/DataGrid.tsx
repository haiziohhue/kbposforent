...
import { BACKENDINFO, Columns, IProps } from "interfaces";
...
export default function CustomDataGrid({
  ...
  let fieldModifier = "";
  if (validFields.includes(query[0]?.field)) {
    fieldModifier = "[value]";
  } else if (
    query[0]?.field === "fournisseur" ||
    query[0]?.field === "client"
  ) {
    fieldModifier = "[name]";
  } else {
    fieldModifier = "";
